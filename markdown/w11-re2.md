...
For the compensation breakdown, a two-part approach can handle every case directly:

1.  Use one regex to locate the whole pay line and retrieve it as a string
2.  Use another regex to `finditer` each of the pay names and ammounts in that string, adding each to the `dict`
